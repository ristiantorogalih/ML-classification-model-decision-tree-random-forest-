...
dalam confusion matrix, threshold default adalah 0.5

jika menurunkan threshold (0.3) -> naikin recall (menaikkan false positive)
jika menaikkan threshold (0.7) -> naikin precision (mengurangi false positive)
...
```
type = raw untuk memunculkan probability sementara type = class untuk memunculkan hasil categoricalnya
...
* **Root Node**: (akar) Cabang pertama, berupa variabel yang paling utama digunakan untuk menentukan nilai target dan menjadi pertimbangan pertama  
* **Internal Node**: (cabang) Cabang kedua dan seterusnya, berupa variabel lain yang digunakan apabila cabang pertama tidak cukup untuk menentukan target    
