...
4. Teks generation: generate alternatif keyword pencarian
5. text summarization: meringkas artikel panjang
...
# convert text to corpus (document dalam bentuk list untuk data text)
...
dalam penggunaan library tm, kita bisa menggunakan fungsi tm_map, untuk melakukan beberapa proses data cleaning
  ...
  - Remove numbers (karena angka tidak mempengaruhi text jadi sebuah spam)
```{r}
...
sms.corpus<-tm_map(sms.corpus, removeWords, stopwords("en"))
sms.corpus[[10]]$content
```

  - Remove punctuations (karena tanda baca bisa muncul di spam/ham dan ga berpengaruh sms jadi spam/ham)
```{r}
sms.corpus <- tm_map(sms.corpus, FUN = removePunctuation)
sms.corpus[[10]]$content
```

  - Stemming (Mengembalikan ke kata dasar untuk kata-kata kerja tertentu yang ada "rules"-nya(-ing, -ed, -es, -ly)) dengan menghilangkan imbuhan
  - lemmatization = mengembalikan ke kata dasar dengan cara lookup kata ke dictionary kata dasarnya
...
hanya dipakai untuk model naive bayes. naive artinya antar prediktornya independent dan sama penting
...
```{r}
confusionMatrix(data=pred_test, reference=test_label, positive="spam")
```
