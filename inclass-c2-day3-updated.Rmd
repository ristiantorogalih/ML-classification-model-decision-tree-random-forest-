...
hanya untuk target variable binary
...
classe: status gerakan pada saat olahraga (1 s.d. 5:tidak benar s.d. benar)

```{r}
levels(fb$classe)
```

Melakukan feature Selection dengan membuang prediktor yang variansinya mendekati 0 (tidak informatif untuk memprediksi target) __> membuang data homogen
```{r}
library(caret)
...
k fold cross validation = membagi data train dan test sebanyak k. nilai k dapat ditentukan sendiri (umumnya 3/5/10)
...
number = banyaknya fold/lipatan (jumlah k)
repeats = berapa kali k-fold diulang/dilakukan

dia akan membuat 15 (5x3) model random forest yg berbeda
...
mtry = jumlah prediktor/ nodes pada pohon (bisa berulang seperti petal length)
